...
class MovieDownloader {
  ...
  async downloadAllMovies() {
    ...
    for (let page = 2; page <= Math.min(totalPages, 100); page++) { // Giới hạn 100 trang để test
    ...
    for (let i = 0; i < pagesToDownload.length; i += BATCH_SIZE) {
      ...
      console.log(`🔄 Batch ${Math.floor(i/BATCH_SIZE) + 1}: Trang ${batch.join(', ')}`);
      ...
      const progress = Math.round(((i + BATCH_SIZE) / pagesToDownload.length) * 100);
  ...
  async saveData() {
    ...
    Object.values(this.categorizedMovies).forEach(category => {
      if (category.movies.length > 0) {
        ...
        console.log(`💾 Đã lưu ${category.movies.length} phim ${category.name} vào ${filename}`);
      }
    });
    ...
    const stats = {
      total_movies: this.allMovies.length,
      categories: Object.values(this.categorizedMovies).map(cat => ({
        name: cat.name,
        slug: cat.slug,
  ...
  async run() {
    try {
      ...
      console.log(`   - Thể loại có phim: ${Object.values(this.categorizedMovies).filter(c => c.movies.length > 0).length}`);
