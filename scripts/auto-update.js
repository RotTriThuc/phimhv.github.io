...
const NotificationIntegration = require('./notification-integration.js');
...
class AutoUpdater {
  ...
  async init() {
    try {
      ...
      console.log(
        `📊 Config: Update every ${this.config.updateInterval / 1000 / 60} minutes`
      );
    } catch (error) {
  ...
  async fetchWithRetry(url, retries = this.config.maxRetries) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        console.warn(
          `⚠️ Attempt ${i + 1}/${retries} failed for ${url}: ${error.message}`
        );
  ...
  compareMovies(existing, updated) {
    ...
    if (existing.episode_current !== updated.episode_current) {
      if (updated.episode_current && updated.episode_current !== 'Full') {
        hasNewEpisode = true;
        changes.push(
          `Episode: ${existing.episode_current} → ${updated.episode_current}`
        );
      }
    }
  ...
  async mergeAndSaveData(existingMovies, newMovies, changes) {
    ...
    const hasSignificantUpdates =
      changes.newMovies.length > 0 || changes.newEpisodes.length > 0;
  ...
  async performUpdate() {
    ...
    try {
      ...
      console.log(
        `📊 Stats: ${this.stats.newMovies} new movies, ${this.stats.newEpisodes} new episodes, ${this.stats.updatedMovies} updates`
      );
      ...
      await this.saveNotification({
        ...
        stats: this.stats,
        hasUpdates:
          changes.newMovies.length > 0 ||
          changes.newEpisodes.length > 0 ||
          changes.updatedMovies.length > 0
      });

      // 🔔 Tạo notifications cho Firebase system và Auto-push to GitHub
      const hasSignificantUpdates =
        changes.newMovies.length > 0 || changes.newEpisodes.length > 0;

      if (hasSignificantUpdates) {
        console.log('\n🔔 Creating notifications for movie updates...');
        try {
          await this.notificationIntegration.createMovieNotifications(
            changes,
            totalMovies
          );
  ...
  async pushToGitHub(updateSummary) {
    ...
    try {
      ...
      const commitMessage = this.config.gitCommitMessage.replace(
        '{updateSummary}',
        updateSummary
      );
      ...
      if (error.message.includes('nothing to commit')) {
        ...
        console.error(
          '🚫 Push rejected by remote (check permissions/conflicts)'
        );
    ...
    try {
      ...
      if (
        ...
        if (currentContent.trim() !== expectedDomain) {
          console.log(
            `🔧 Fixing CNAME file: "${currentContent.trim()}" → "${expectedDomain}"`
          );
  ...
  async syncWithRemote(projectRoot) {
    try {
      ...
      if (hasUncommittedChanges) {
        ...
        execSync('git stash push -m "Auto-stash before sync"', {
          cwd: projectRoot,
          stdio: 'inherit'
        });
      }
      ...
      if (hasUncommittedChanges) {
        try {
          ...
          console.warn(
            '⚠️ Could not restore stashed changes:',
            stashError.message
          );
  ...
  async startScheduler() {
    console.log(
      `⏰ Starting scheduler with ${this.config.updateInterval / 1000 / 60} minute intervals`
    );
