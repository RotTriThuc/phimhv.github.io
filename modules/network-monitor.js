...
export class NetworkMonitor {
  ...
  async measureBandwidth(detailed = false) {
    const testSize = detailed ? NETWORK_CONFIG.fullTestSize : NETWORK_CONFIG.quickTestSize;
    ...
    try {
      ...
      const response = await fetch(`${testUrl}?t=${Date.now()}&size=${testSize}`, {
        method: 'HEAD',
        cache: 'no-cache'
      });
  ...
  updateQualityRecommendation() {
    ...
    const connectionPriority = NETWORK_CONFIG.connectionPriority[this.connectionType] || 2;
  ...
  updateStats() {
    if (this.measurements.length > 0) {
      const totalBandwidth = this.measurements.reduce((sum, m) => sum + m.bandwidth, 0);
