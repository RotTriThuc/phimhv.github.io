...
export class EnhancedPerformanceMonitor {
  ...
  recordVideoMetric(data) {
    ...
    if (data.type === 'session_end') {
      this.realTimeStats.activeVideoSessions = Math.max(
        0,
        this.realTimeStats.activeVideoSessions - 1
      );
    }
  ...
  checkNetworkThresholds(metric) {
    ...
    if (metric.bandwidth && metric.bandwidth < thresholds.slowNetworkSpeed) {
      this.addAlert(
        'network',
        `Slow network detected: ${metric.bandwidth.toFixed(2)} Mbps`,
        'warning'
      );
    }
    ...
    if (metric.latency && metric.latency > thresholds.highLatency) {
      this.addAlert(
        'network',
        `High latency detected: ${metric.latency.toFixed(0)}ms`,
        'warning'
      );
    }
  }
  ...
  checkVideoThresholds(metric) {
    ...
    if (
      metric.type === 'buffer_health' &&
      metric.value < thresholds.lowBufferHealth
    ) {
      this.addAlert(
        'video',
        `Low buffer health: ${metric.value.toFixed(1)}s`,
        'warning'
      );
    }
  ...
  checkApiThresholds(metric) {
    ...
    if (
      metric.responseTime &&
      metric.responseTime > thresholds.slowApiResponse
    ) {
      this.addAlert(
        'api',
        `Slow API response: ${metric.responseTime.toFixed(0)}ms for ${metric.url}`,
        'warning'
      );
    }
    ...
    if (metric.type === 'error') {
      this.addAlert(
        'api',
        `API error: ${metric.message} for ${metric.url}`,
        'error'
      );
    }
  }
  ...
  generateReport() {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent('performance-report', { detail: report })
      );
    }
