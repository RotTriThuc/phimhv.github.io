...
export class VideoCacheManager {
  ...
  async calculateCacheSize() {
    ...
    try {
      ...
      request.onsuccess = () => {
        ...
        this.cacheSize = segments.reduce((total, segment) => total + segment.size, 0);
    ...
    try {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor && freedSpace < requiredSpace) {
          ...
          if (age > 60000 || freedSpace < requiredSpace) { // 1 minute minimum age
  ...
  async deleteSegments(segments) {
    try {
      ...
      Logger.info('🗑️ Evicted segments:', {
        ...
        freedSpace: this.formatBytes(segments.reduce((sum, s) => sum + s.size, 0))
      });
  ...
  async prefetchSegments(videoId, currentSegmentIndex, segmentUrls) {
    ...
    const endIndex = Math.min(startIndex + CACHE_CONFIG.prefetchSegments, segmentUrls.length);
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (await this.hasSegment(segmentId) || this.prefetchQueue.includes(segmentId)) {
        continue;
      }
  ...
  shouldPrefetch() {
    ...
    if (CACHE_CONFIG.prefetchOnlyOnWifi && networkInfo.connectionType !== 'wifi') {
      return false;
    }
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          ...
          if (expiredSegments.length > 0) {
            ...
            Logger.info('🧹 Cleaned up expired segments:', expiredSegments.length);
          }
        }
      };
  ...
  getStats() {
    ...
    return {
      ...
      cacheUsage: ((this.cacheSize / CACHE_CONFIG.maxCacheSize) * 100).toFixed(1) + '%',
